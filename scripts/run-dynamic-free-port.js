...
conn.on("ready", async () => {
  try {
    console.log("\n--- DYNAMICALLY ALLOCATING KERNEL FREE PORT ---");

    const dynamicLaunch = `
      cd /home/u375327955/pqn-app
      export PATH=/opt/alt/alt-nodejs22/root/usr/bin:$PATH

      # 1. Ask Linux Kernel for a guaranteed free port
      ...
      echo "Allocated Free Port: $FREE_PORT"

      # 2. Update .env
      sed -i "s/PORT=.*/PORT=$FREE_PORT/g" .env

      # 3. Update index.php reverse proxy
...
EOF
    ...
    const res = await runCommand(dynamicLaunch);
